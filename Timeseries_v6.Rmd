...
MPCAtimeseries1 <- read.csv("Raw Data/MPCA_MassDownload_20240521.csv")
...
In summer 2024 we found that some organizations reported secchi depth values in ft but reported the units as m
This seems to happen inconsistently from year to year and org to org, so it's hard to find all the problems
The problems fixed here were identified by secchi depths reported that were greater than the max depth of the lake
This problem is particularly hard to catch when secchi is < 1m 
(because it's hard to tell if a year-to-year change in secchi of 0.3m -> 1m is a real signal or a unit error)
I'm in communication with MCPA about this and they are fixing the database, so if we re-scrape data we may need to skip this step

```{r}
Secchi_flag <- read.csv("Raw Data/Secchi_WrongUnit.csv") %>%
  mutate(DOW = fixlakeid(DOW))


#pull out all samples that are reported as m but are actually ft
flag_list <- list()

for (i in 1:dim(Secchi_flag)[1]) {
  flag_sub <- MPCAOrg %>%
  filter(DOW == Secchi_flag$DOW[i] & 
         collectingOrg == Secchi_flag$collectingOrg[i] & 
         sampleYear == Secchi_flag$sampleYear[i] &
         parameter == "Depth, Secchi disk depth")
  
  flag_list[[i]] <- flag_sub
}

flags = do.call(rbind, flag_list)

#remove these samples from the dataset
MPCAnoflags <- anti_join(MPCAOrg, flags, by = c("collectingOrg", "parameter", "result", "sampleDate", "DOW", "gtlt"))

#fix the unit problem
flags_fixed <- flags %>%
  mutate(result = result/3.281)
  
#add the data back
MPCAOrg0 <- rbind(flags_fixed, MPCAnoflags)
...
#I also manually fixed a ft instead of m secchi depth error for a measurement on 5/17/2021 for 82000200
...
#because there are not multiple sites we don't have to collapse spatial surveys and we can just average any duplicate reports
  ...
  summarise(result = mean(result), gtlt = mean(gtlt)) %>%
  ungroup() %>%
  filter(result < 5000) #remove very high secchi values
...
AllData <- MPCA_RC_SWWD_CCWMO_RPBCWD_MCES_ACD_EG_DNR_NMCWD %>% 
  ...
  mutate(parameter = if_else(parameter == "DIN.mg_L", "NOX.mg_L", parameter)) #rename DIN.mg_L to NOX.mg_L because it doesn't contain NH4 data
