...
2) The same organization reporting multiple samples for one site
3) Samples from multiple sites on the same lake on the same day (spatial survey)
...
1) If the same result value is reported twice as both < and no flag, I will keep the value with a flag. It is not uncommon for samples at the detection to not be reported as such (~117 samples)
2) Once this is handled, if there are a mix of result values and a mix of < and no flag I will use the detection limit as the value for averaging, but I will include a flag that indicates BDL values have been mixed in
...
AllData <- MPCA_RC_SWWD_CCWMO_RPBCWD_MCES_ACD_EG_DNR_NMCWD %>% 
  ...
  mutate(dataSource = if_else(dataSource == "MCPA", "MPCA", dataSource))
